...
\begin{description}
\item [{type=<type>}] Defines the course type. Predefined values are:
...
\item [{room=<room>}]~
\item [{startdate=<startdate>}] The date of the first session (the argument
      \emph{must} have the form DD/MM/YYYY). If no startdate is given,
      \jfcsmacro{today} is used.
\item [{interval=<interval>}] The interval (in days) between two sessions.
