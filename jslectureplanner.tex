...
\date{Version 1.0dev, 2015/02/26}
...
formats are German. However, it is possible to customize, extend and
translate them to other languages. This is described later in this manual.
...
\item \jcsmacro{SetAutoOffset\{<n>\}\{<days>\}}: Automatically shift all subsequent sessions
      ...
      two regular sessions and then a pause until the next week (for instance, a Tuesday + Thursday course could
      be defined via \jcsmacro{LecInterval\{2\}} and \jcsmacro{SetAutoOffset\{2\}\{3\}})
\end{itemize}
...
\begin{itemize}
\item Forthcoming (v.\,1.0): Lecture types and term types are not hardcoded anymore and can
    be freely modified and extended via \jcsmacro{DefLecType} and \jcsmacro{DefSemType};
    add option to add the instructor's name to both the course (via
    \jcsmacro{LecInstructor} or \joption{instructor} package option) and to specific sessions
    (via \joption{instructor} option of \jcsmacro{NewSession});
    add option to add names of presenting students to a session (via \joption{presstudents}
    option);
    add option to record the office number (\jcsmacro{SetOfficeNumber} or \joption{officenumber}
    package option);
    fix expansion issue in \joption{options} option of \jcsmacro{makebeamerprogram};
    add option to automatically shift all subsequent sessions by a certain value after every
    \emph{n}th session (\jcsmacro{SetAutoOffset}).
