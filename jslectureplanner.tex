...
\end{itemize}
\item \jcsmacro{SetBreak[<span>]\{Reason\}}: Add a semester break
(e.\,g., holidays) that affects one session. By means of the optional
argument, the real time span of the break might be given.
The macro increments the internal counters respectively and expands
in the program to
...
a semester break that affects more than one session. The output in the program
is identical to the \jcsmacro{SetBreak} macro, but the counter incrementation
can be adjusted to the needed time span.
...
\item \joption{units=<n>}: Number of interval units the break takes. One \emph{unit}
    ...
    incremented by one.\footnote{That is: \jcsmacro{SetBreak\{Reason\}} =
    \jcsmacro{SetBreaks\{Reason\}} = \jcsmacro{SetBreaks[units=1]\{Reason\}}.}
