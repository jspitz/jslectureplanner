...
\date{Version 0.9dev, 2015/01/17}
...
\begin{itemize}
\item \joption{units=<int>}: Number of interval units the break takes. One \emph{unit}
    is the number of days as specified by the \joption{interval} option (one week by
    default\footnote{The former option \joption{weeks} is deprecated a of v. 0.9,
    ...
    incremented by one.
\item \joption{span=<time span>}: time span (verbally output on the program)
\end{itemize}
...
\item forthcoming (v. 0.9): Add a way to adjust the interval between two sessions
    (\joption{interval} package option and \jcsmacro{LecInternal} macro); rename option \joption{weeks}
