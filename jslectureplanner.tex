...
\date{Version 1.4dev, 2016/10/07}
...
\item \jcsmacro{sesinstructor}: Current session's instructor's name. If you want to output this name only if it differs from the general instructor (\jcsmacro{lecinstructor}), you can use the check \jcsmacro{ifbool\{othersesinstructor\}\{\textbackslash sesinstructor\}\{\}}
\item \jcsmacro{sesshortinstructor}: Current session's instructor's short name
...
\item Forthcoming (v. 1.4):
	\begin{itemize}
		...
		\item Add boolean check for identical lecture and session instructor.
	\end{itemize}
