...
\title{The \texttt{jslectureplanner} bundle}

\author{Jürgen Spitzmüller%
\thanks{Please report issues via \protect\url{https://github.com/jspitz/jslectureplanner}.}}

\date{Version 1.1dev, 2015/07/07}

\maketitle

\begin{abstract}
\noindent The \textsf{jslectureplanner} bundle provides a collection of packages
I have written in order to facilitate (and somewhat systematize)
the planning of my university courses and to generate the basic material.
Currently, it consists of the main, title-giving package \textsf{jslectureplanner}
that provides an interface to central, partially dynamically calculated course metadata,
and two smaller companion packages: \textsf{jsmembertable}, a package to generate
course member tables and \textsf{jspreslist}, a package to generate a list of student
presentations. This manual documents all these packages.
\end{abstract}
...
\part{The \emph{jslectureplanner} package}

This part of the manual describes the basic user interface of the
\textsf{jslectureplanner} package.
...
The basic idea of the \textsf{jslectureplanner} package is that you record
...
To further facilitate the planning, the \textsf{jslectureplanner} package
...
\section{Requirements of \emph{jslectureplanner}}

The following packages are required by \textsf{jslectureplanner}: \textsf{etoolbox}, \textsf{advdate},
...
\section{The \emph{jsmembertable} package}

This section of the manual describes the user interface of the \textsf{jsmembertable}
...
university course (a table where your students can
fill in names, student IDs and e-mail adresses) and a table to record their presence,
should this be a requisite at your institute.

The package somewhat integrates with \textsf{jslectureplanner} and can use the
...
Like for \textsf{jslectureplanner}, the default strings are German.
However, it is possible to customize and translate them.
This is described later in this manual.


\subsection{Requirements of \emph{jsmembertable}}

The following packages are required by \textsf{jsmembertable}: \textsf{ifthen}, \textsf{calc},
\textsf{longtable}, \textsf{hhline}; \textsf{jsmembertable} can be used independently of
\textsf{jslectureplanner} to some degree. The latter package is only
required for specific features (see below) and must be loaded explicitly.

\subsection{Loading the package}

The package is loaded like this:
\begin{quote}
\jcsmacro{usepackage\{jsmembertable\}}
\end{quote}
...
\begin{quote}
\jcsmacro{makemembertable\{<number of students>\}}
\end{quote}
The columns are fixed: \emph{student id}, \emph{name}, \emph{e-mail address}.
If you want to have more or less columns, you need to redefine the
\jcsmacro{makemembertable} command.
The header strings, however, can easily be adjusted by redefining the following three
...
\end{lstlisting}
If you use the \textsf{jslectureplanner} package, you can use the \textsf{jslectureplanner}
style files in order to store those redefinitions globally. Of course, you need
to load \textsf{jslectureplanner} with the respective \joption{style} option then
(\emph{before} \textsf{jsmembertable}).
See above section~\ref{sec:styles} for details on styles.

\subsection{Generating a presence table}

Presence tables consist of a row where your students fill in their name and rows
for each session of your course where the students can sign and mark their presence
in the respective session.
The package provides two kinds of presence tables. The first one requires the
\textsf{jslectureplanner} package and uses the course metadata in order
to generate a matching number of rows with the respective session date as row header.
If you use \textsf{jslectureplanner} anyway to generate your course material,
this is the most convenient approach.

You must load the \textsf{jslectureplanner} package and input the metadata file
\emph{before} \textsf{jsmembertable} if you want to use this feature (the package yields
a warning otherwise). Then just insert:

\begin{quote}
\jcsmacro{makeprestable\{<number of students>\}}
\end{quote}
The number of sessions is automatically calculated from the metadata. If you want a different
number, you can use the optional argument of the command:

\begin{quote}
\jcsmacro{makeprestable[<number of sessions>]\{<number of students>\}}
\end{quote}
The second kind of presence table is provided via the starred version of the command:

\begin{quote}
\jcsmacro{makeprestable*[<number of sessions>]\{<number of students>\}}
\end{quote}
This one also works without \textsf{jslectureplanner}. As opposed to the unstarred version,
it does not print the session dates as column headers, but ``Session <nr>'' with an
accelerating number. Like with the unstarred version, if the \textsf{jslectureplanner}
metadata is used/available, the number of sessions is calculated from the metadata,
if you do not pass a different number via the optional argument.
If the \textsf{jslectureplanner} metadata is not
used/available, and no column number is given via the optional argument, the table will use
a preset number of sessions, namely 15.

For courses of 2--16 sessions, the package tries to produce reasonably sized tables that
fit the page width. From 7 sessions upwards, two separate tables, each for half a semester,
are produced.

The header strings can be adjusted by redefining the following
macros (using \jfcsmacro{renewcommand*}), for instance like this:
...
\end{lstlisting}
Again, if you use \textsf{jslectureplanner}, you can use the \textsf{jslectureplanner}
style files in order to store those redefinitions globally. Of course, you need
to load \textsf{jslectureplanner} with the respective \joption{style} option then
(\emph{before} \textsf{jsmembertable}).
See above section~\ref{sec:styles} for details on styles.
...
linked to \textsf{jslectureplanner}'s \jcsmacro{lectypesession} value if
\textsf{jslectureplanner} is loaded before \textsf{jsmembertable}.


\section{The \emph{jspreslist} package}

This section of the manual describes the user interface of the \textsf{jspreslist}
package. Please also refer to the example files included in the bundle.

\subsection{Aim of this package}

This package provides a way to generate lists of student presentations.
It uses the metadata of the \textsf{jslectureplanner} package 
(see part~I of this manual for details on metadata).


\subsection{Requirements of \emph{jspreslist}}

The following packages are required by \textsf{jspreslist}: \textsf{ifthen}, \textsf{calc},
\textsf{etoolbox}, \textsf{jslectureplanner}.

\subsection{Loading the package}

The package is loaded like this:
\begin{quote}
\jcsmacro{usepackage\{jspreslist\}}
\end{quote}
Currently, no package options are available.

\subsection{Generating a blank presentation list}

In order to schedule the presentations of your students in the first session, you might
want a list just consisting the session dates and titles with enough space to fill in
student names. Given that your metadata is set up correctly and that
\textsf{jslectureplanner} and the metadata are loaded \emph{before} \textsf{jspreslist},
it is as simple as

\begin{quote}
\jcsmacro{makepreslist*}
\end{quote}
If you want to start your presentations only from a given session number, you can use

\begin{quote}
\jcsmacro{setfirstsession\{<session number>\}}
\end{quote}
to set the first session number to be printed on the list.

\subsection{Generating a filled-in presentation list}

If you have collected the names of the presenting students, you can assign them in the
metadata to specific sessions by the \joption{presstudents} option of the
\jcsmacro{NewSession} command (see above sec.~\ref{ses-add}). If you have done so,
and given that \textsf{jslectureplanner} and the metadata are loaded \emph{before}
\textsf{jspreslist}, the unstarred macro

\begin{quote}
\jcsmacro{makepreslist}
\end{quote}
will output a list using the names of the presenting students. If no presenting
students have been assigned for a session, an em-dash (---) will be output instead.
Again,if you want to start your presentations only from a given session number, you can use
...
For beamer presentations, a specific macro

\begin{quote}
\jcsmacro{makebeamerpreslist}
\end{quote}
is provided (no starred version). The main difference is the more compact design that makes
this fit better on a slide.
...
\begin{itemize}
\item Forthcoming (v. 1.1dev):
     \begin{itemize}
       \item Include and document the \textsf{jsmembertable} and \textsf{jspreslist}
             packages.
     \end{itemize}
