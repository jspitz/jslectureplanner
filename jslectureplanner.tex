...
\end{itemize}
%
Furthermore, if your material contains information that is specific to a given term, you can use the test
\verb|\IfSemesterTF{<term>}{<true>}{<false>}|, where \joption{<term>} is the term specifier used in
course metadata specification. For example:
\begin{lstlisting}[language={[LaTeX]TeX},basicstyle={\small\ttfamily},
	frame=single,escapechar=\$]
\IfSemesterTF{ws}{Winter}{Spring} break
\end{lstlisting}
...
\item \joption{size=\jparam{size command}}: Adjust the size of the program.
...
Vít Starý Novotný made me aware of some \textsf{expl3} coding flaws -- thank you!
...
\begin{itemize}
\item 2025/10/02 (v.\,1.15):
\begin{itemize}
	\item Fix some expl3 code.
	\item Add \jcsmacro{IfSemesterTF} check (see sec.~\ref{sec:csdata}).
	\item Add \joption{size} option to \jcsmacro{makebeamerprogram}.
	\item Fix expansion issue with \textsf{datatool} \texttt{keys} in \textsf{jsmembertable}.
\end{itemize}
