...
Currently, it consists of the main, title-giving package \jslp\
...
connected to a verbose string (such as ``Seminar''), a short form (such as
...
\item [\joption{year=<year>}] Year of the course
\item [\joption{uni=<university>}] Your university
\item [\joption{institute=<institute>}] Your institute/department
\item [\joption{room=<room>}] The room the course takes place (generally)
...
\begin{description}
\item [\joption{instructor=<name>}] Instructor's name
\item [\joption{platform=<data>}] Information (e.\,g., URL) of the e-learning platform
\item [\joption{officehours=<data>}] Date of your office hours
\item [\joption{officenumber=<data>}] Your office number
\end{description}
...
\subsection{Requirements of \jsmt}
...
\jslp\ to some degree. The latter package is only
...
\jslp\ package and uses the course metadata in order
to generate a matching number of columns with the respective session date as
column header.
If you use \jslp\ anyway to generate your course material,
this is the most convenient approach.

You must load the \jslp\ package and input the metadata file
\emph{before} \jsmt\ if you want to use this feature (the package yields
...
\jcsmacro{makeprestable*[<number of sessions>]\{<number of students>\}}
\end{quote}
This one also works without \jslp. As opposed to the unstarred version,
it does not print the session dates as column headers, but ``Session <nr>'' with an
accelerating number. Like with the unstarred version, if the \jslp\
