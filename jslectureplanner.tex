...
\date{Version 1.3dev, 2016/07/21}
...
If you need to change the date format, redefine the date styles \textsf{vldate}
(for the long format) and \textsf{vlshortdate} (for the short format),
...
\DTMnewdatestyle{vldate}{%
\renewcommand*{\DTMdisplaydate}[4]{\DTMtwodigits{##3}.\,\DTMtwodigits{##2}.~##1}%
\renewcommand*{\DTMDisplaydate}{\DTMdisplaydate}%
}

\DTMnewdatestyle{vlshortdate}{%
\renewcommand*{\DTMdisplaydate}[4]{\DTMtwodigits{##3}.\,\DTMtwodigits{##2}.}%
\renewcommand*{\DTMDisplaydate}{\DTMdisplaydate}%
}
\end{lstlisting}
\jfcsmacro{DTMtwodigits\{\#\#3\}} represents the day of the month (using always two digits, i.\,e., 01 not 1), \jfcsmacro{DTMtwodigits\{\#\#2\}} represents the month (also in two-digit form) and \jfmacro{\#\#1} represents the year. Please refer to the datetime2 manual for details. If you want to redefine the styles, use \jfcsmacro{DTMrenewdatestyle} and change the definition accordingly.

Note that the date redefinition syntax used up to \jslp\ 1.2 is no longer supported, since it relied on the \textsf{datetime} package, which is deprecated (and superseded by \textsf{datetime2}). The \jslp\ package will issue a warning if you try to use the old syntax.
...
\begin{itemize}
\item Forthcoming (v. 1.3):
     \begin{itemize}
     	\item Use \textsf{datetime2} instead of \textsf{datetime} and \textsf{advdate}.
