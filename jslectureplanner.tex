...
\date{Version 1.8dev, 2019/01/08}
...
\item \joption{exam=\jparam{true|false}}: If \joption{true}, this session is put to the
      exam rather than the program list.
...
\end{itemize}
%
Note that if you are looking for exam session particularly, you can also use
...
will return the date of the second exam session, notwithstanding which global
session number this exam has.
...
\begin{itemize}
\item forthcoming (v. 1.8):
	\begin{itemize}
		\item Add \joption{extradate} option to \jcsmacro{NewSession} to allow for fix dates.
		\item Add possibility to enter and retrieve (separate) exam dates (option \joption{exam}
		      of \jcsmacro{NewSession}, \jcsmacro{makeexamprogram} and \jcsmacro{makebeamerexamprogram}),
		      and by the means of \jcsmacro{examsesno}.
