...
\date{Version 1.7dev, 2018/12/01}
...
\begin{lstlisting}[language={[LaTeX]TeX},basicstyle={\small\ttfamily},frame=single,moretexcs={[1]{frametitle}}]
\begin{frame}<overlayarg>[label=beamerprogram<n>,<bfoptions>]
\frametitle{<Heading>}
...
\item \joption{overlayarg=<overlay specification>}: Beamer frame overlay specification
     (to be specified without angle brackets).
\end{itemize}
...
\begin{itemize}
\item Forthcoming (v. 1.7):
	\begin{itemize}
		\item Allow \joption{options} in \jcsmacro{makebeamerprogram} also with \joption{blocksonly}.
		\item Add \joption{overlayarg} option to \jcsmacro{makebeamerprogram}.
	\end{itemize}
