...
\item \joption{cologne} provides some specifics current at the University of Cologne
     ...
     \begin{itemize}
      \item Additional course types: \joption{ak} (\emph{Arbeitskurs}),
      \joption{as} (\emph{Aufbauseminar}), \joption{es} (\emph{Einführungsseminar}),
      \joption{evl} (\emph{Einführungsvorlesung}), \joption{hs} (\emph{Hauptseminar}).
     \end{itemize}
