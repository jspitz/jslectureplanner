...
\date{Version 1.2dev, 2016/05/08}
...
\subsection{Generating session-specific information for the next/previous session(s)}

Depending on the value of \jcsmacro{ThisSession}, you can retrieve the following
information for sessions following or preceding the current session via the following macros:
\begin{itemize}
	\item \jcsmacro{NextSessionTitle[<offset>]}: Outputs the main title
	of the next session
	\item \jcsmacro{NextSessionFullTitle[<offset>]}: Outputs the full
	title (main title, separator, subtitle) of the next session
	\item \jcsmacro{NextSessionShortTitle[<offset>]}: Outputs the short
	title of the next session
	\item \jcsmacro{NextSessionDate[<offset>]}: Outputs the date of the next session
	\item \jcsmacro{NextSessionShortDate[<offset>]}: Outputs the short
	date (no year) of the next session
	\item \jcsmacro{NextSessionInstructor[<offset>]}: Outputs the name of the instructor of the next session
	\item \jcsmacro{NextSessionPresStudents[<offset>]}: Outputs the name of the students presenting
	in the next session
	\item \jcsmacro{NextSessionRoom[<offset>]}: Outputs the room of the next session
\end{itemize}
Via the numerical \joption{<offset>} option (default: 1), you can specify which session, relative to the current one, you want to output. Negative values are valid; \jcsmacro{NextSessionTitle[-1]} would output the title of the previous session.
...
\begin{itemize}
\item Forthcoming (v. 1.2):
    \begin{itemize}	
	\item Fix line breaking with empty session titles before course breaks. 
	\item New commands for getting next session data:
	   \begin{itemize}	
		  \item \jcsmacro{NextSessionFullTitle}
		  \item \jcsmacro{NextSessionDate}
		  \item \jcsmacro{NextSessionShortTitle}
		  \item \jcsmacro{NextSessionTitle}
		  \item \jcsmacro{NextSessionShortDate}
		  \item \jcsmacro{NextSessionRoom}
		  \item \jcsmacro{NextSessionInstructor}
		  \item \jcsmacro{NextSessionPresStudents}
	  \end{itemize}
    \end{itemize}
