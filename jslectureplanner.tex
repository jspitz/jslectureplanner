...
\date{Version 1.1dev, 2015/07/14}
...
of columns with the respective session date for the column headers. The second one also works
...
The first presence table variant (with real session dates) is produced with the macro
...
\end{quote}
The second kind of presence table (with session numbers) is provided via the starred version
of the command:
...
The optional argument can be used to override this calculation.
If the \jslp\ metadata is not used/available, and no column number is given via the optional
argument, the table will use a preset number of sessions, namely 15. The starred version also
uses the external student data if the \joption{datafile} option has been used.
